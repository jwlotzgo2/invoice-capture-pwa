...
const css = `
  * { box-sizing:border-box; }
  ...
  .detail-page { min-height:100svh;background:${T.bg};font-family:Inter, system-ui, sans-serif,Inter, system-ui, sans-serif;color:${T.text}; }
  ...
  .detail-header { background:${T.surface};border-bottom:1px solid ${T.border};padding:12px 16px;display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:40;box-shadow:0 0 20px rgba(138,138,138,0.08); }
  ...
  .t-pill { padding:6px 12px;border-radius:4px;border:1px solid ${T.border};background:transparent;color:${T.textDim};font-family:Inter, system-ui, sans-serif;font-size:11px;letter-spacing:0.2px;cursor:pointer;transition:all 0.15s;text-transform:none; }
  .t-pill:hover { border-color:${T.blue};color:${T.text}; }
  .t-pill.active { border-color:${T.yellow};background:${T.yellowGlow};color:${T.yellow}; }
  .detail-label { font-size:10px;letter-spacing:0.3px;color:${T.text};text-transform:none;margin-bottom:4px; }
  .detail-value { font-size:14px;color:${T.text};font-family:Inter, system-ui, sans-serif; }
  .badge { display:inline-flex;align-items:center;padding:3px 10px;border-radius:4px;font-size:10px;text-transform:none;letter-spacing:0.2px;border:1px solid;font-family:Inter, system-ui, sans-serif; }
      ...
      <div className="detail-page">
        ...
        <main style={{padding:16,paddingBottom:80}}>
          ...
          <div style={{display:'flex',gap:6,marginBottom:12,flexWrap:'wrap'}}>
            ...
            {/* Match status */}
            {hasLines && totalsMatch && <span className="badge" style={{background:'rgba(134,239,172,0.12)',color:T.success,borderColor:T.success}}>✓ Match</span>}
            {hasLines && !totalsMatch && <span className="badge" style={{background:'rgba(252,165,165,0.12)',color:T.error,borderColor:T.error}}>⚠ {fmtZAR(diff)} Off</span>}
              ...
              {lineItems.length>0 && (
                <div className="t-card">
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
                    ...
                    {invoiceTotal>0 && (
                      <span style={{fontSize:10,letterSpacing:1,padding:'3px 8px',borderRadius:4,fontFamily:'Inter, system-ui, sans-serif',textTransform:'none',background:totalsMatch?'rgba(134,239,172,0.12)':'rgba(252,165,165,0.12)',color:totalsMatch?T.success:T.error,border:`1px solid ${totalsMatch?T.success:T.error}`}}>
                  ...
                  {lineItems.map((item,i)=>(
                    <div key={i} style={{paddingBottom:10,marginBottom:10,borderBottom:i<lineItems.length-1?`1px solid ${T.border}`:'none'}}>
                      ...
                      <div style={{display:'flex',gap:12,fontSize:12,color:T.textDim,fontFamily:'Inter, system-ui, sans-serif'}}>
                  ...
                  <div style={{display:'flex',justifyContent:'space-between',paddingTop:8,borderTop:`1px solid ${T.border}`}}>
                    <span style={{fontSize:11,color:T.textMuted,letterSpacing:0.5,textTransform:'none'}}>Items Total</span>
                    <span style={{fontFamily:'Inter, system-ui, sans-serif',color:T.yellow,fontSize:14}}>{fmtZAR(itemsTotal)}</span>
                  </div>
                </div>
              )}
